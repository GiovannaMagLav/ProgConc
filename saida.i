...
int main(int argc, char *argv[]){
    ...
    if(argc<3){
        fprintf(__stderrp, "ERRO de entrada, digite <%s> <dimensao do vetor> <numero de threads>\n", argv[0]);
        return 1;
    }
    dim = atoi(argv[1]);
    ...
    if(inicia_vetor(dim, &vetor)!=0){
        fprintf(__stderrp, "ERRO de alocação de memória");
        return 1;
    }
    ...
    for(short int i=0; i<nthreads; i++){
        ...
        if(args == ((void *)0)){
            fprintf(__stderrp, "ERRO de alocação de memória para argumentos da thread %hd\n", i);
            return 2;
        }
        ...
        if(pthread_create(&tid[i], ((void *)0), tarefa, (void *) args)){
            fprintf(__stderrp, "ERRO na criação da thread %hd\n", i);
            return 3;
        }
    }
    ...
    for(short int i=0; i<nthreads; i++){
        if(pthread_join(tid[i], ((void *)0))){
            fprintf(__stderrp, "ERRO de join da thread %hd", i);
            return 4;
        }
    }
    ...
    if(corretude(dim, vetor)){
        fprintf(__stderrp, "ERRO no vetor de saída\n");
    }
    else
        puts("Arrasou, diva!");
